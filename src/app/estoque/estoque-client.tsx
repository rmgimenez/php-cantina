...
export default function EstoqueClient() {
  ...
  const produtosFiltrados = produtos.filter((produto) => {
    ...
    const estoqueBaixoMatch =
      !filtroEstoqueBaixo || produto.estoqueAtual <= produto.estoqueMinimo;
    ...
    <div className="container-fluid py-4">
      ...
      {produtosEstoqueBaixo.length > 0 && (
        <WarningAlert onClose={() => {}}>
          Existem <strong>{produtosEstoqueBaixo.length}</strong> produto(s) com
          estoque baixo. Verifique a lista abaixo ou filtre por "Mostrar apenas
                ...
                historico.map((item) => {
                  ...
                  return (
                    <tr key={item.id}>
                      ...
                      <td>
                        <span
                          className={getTipoMovimentacaoClass(
                            item.tipoMovimentacao
                          )}
                        >
                          {getTipoMovimentacaoIcon(item.tipoMovimentacao)}{" "}
                          {item.tipoMovimentacao}
                        </span>
                      </td>
